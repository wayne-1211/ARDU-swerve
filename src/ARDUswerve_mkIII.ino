...
void loop() {
  ...
  if ((xpos > noise || xpos < -noise) && (ypos > noise || ypos < -noise) || (tpos > 10 || tpos < -10)){ //noise control
    ...
    FL.write((theta + Lturn) > 0 ? theta + Lturn : theta + Lturn + 180);
    FR.write((theta - Rturn) > 0 ? theta - Rturn : theta - Rturn + 180);
    BL.write((theta - Lturn) > 0 ? theta - Lturn : theta - Lturn + 180);
    BR.write((theta + Rturn) > 0 ? theta + Rturn : theta + Rturn + 180);
  } else {
    FL.detach();
    FR.detach();
    BL.detach();
